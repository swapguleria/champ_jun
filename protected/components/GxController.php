...
abstract class GxController extends Controller
    ...
    public function loadModel($key, $modelClass)
        ...
        if (is_array($key))
            ...
            if (key($key) === 0)
                ...
                if (count($key) === 1) return $this->loadModel($key [0], $modelClass);

                // Now we will use the composite PK.
                // Check if the table has composite PK.
                $tablePk = $staticModel->getTableSchema()->primaryKey;
                if (!is_array($tablePk)) throw new CHttpException(400, Yii::t('giix', 'Your request is invalid.'));

                // Check if there are the correct number of keys.
                if (count($key) !== count($tablePk)) throw new CHttpException(400, Yii::t('giix', 'Your request is invalid.'));
            ...
            if (is_array($tablePk))
                ...
                if (!is_string($key)) throw new CHttpException(400, Yii::t('giix', 'Your request is invalid.'));

                // There must be a PK separator in the key.
                if (stripos($key, GxActiveRecord::$pkSeparator) === false) throw new CHttpException(400, Yii::t('giix', 'Your request is invalid.'));
        ...
        if ($model === null) throw new CHttpException(404, Yii::t('giix', 'The requested page does not exist.'));
     ...
     *        	Since Yii 1.1.7, htmlOptions (in {@link CHtml::activeCheckBoxList})
    ...
    protected function getRelatedData($form, $relations, $uncheckValue = '')
        {
        if ($uncheckValue === null) throw new InvalidArgumentException(Yii::t('giix', 'giix cannot handle automatically the POST data if "uncheckValue" is null.'));

        $relatedPk = array();
        foreach ($relations as $relationName => $relationData)
            {
            if (isset($form [$relationName]) && (($relationData [0] == GxActiveRecord::HAS_MANY) || ($relationData [0] == GxActiveRecord::MANY_MANY))) $relatedPk [$relationName] = $form [$relationName] === $uncheckValue ? null : $form [$relationName];
            }
        return $relatedPk;
        }
    ...
    public function AddPanel($title, $objects, $relations, $view, $model = null, $addMenu = true)
        ...
        if ($objects)
            ...
            if ($objects instanceof CActiveDataProvider) $dataProvider = $objects;
    ...
    public function EndPanelRight()
        ...
        $this->widget('bootstrap.widgets.TbTabs', array(
            ...
            'tabs' => $this->tabs_data,
            'htmlOptions' => array(
                'class' => 'tabbable tabs-right well'
            )
        ));
        }
    ...
    protected function richTextEditor()
        {
        return 3; // 0 // mean no rich text editor
        }
    ...
    public function actionUpload($name = 'filedata')
        ...
        if (isset($uploaded_file))
            ...
            $data = array(
                'err' => '',
                'msg' => $this->CreateUrl('download', array(
...
//        throw new CHttpException(404, Yii::t('app', 'File not found'));
    ...
    public function sendFile($thumbfile)
        {
        if (file_exists($thumbfile))
            ...
            header("Content-Disposition: attachment; filename=\"" . $thumbfile . "\";");
    ...
    protected function processActions($model = null)
        ...
        $this->actions [] = array(
            'label' => Yii::t('app', 'Follow'),
            'icon' => 'icon-hand-up icon-white',
            'url' => array(
                'user/follow',
                'id' => $model->id,
                'model' => get_class($model)
            ),
            'visible' => !$follow && !User::isSelf($model) && !Yii::app()->user->isGuest
        );
        $this->actions [] = array(
            'label' => Yii::t('app', 'Following'),
            'icon' => 'icon-hand-down icon-white',
            'url' => array(
                'user/follow',
                'id' => $model->id,
                'model' => get_class($model)
        ...
        $this->actions [] = array(
            ...
            'visible' => !$like && !User::isSelf($model) && !Yii::app()->user->isGuest
        );
        $this->actions [] = array(
            'label' => Yii::t('app', 'Liked'),
            ...
            'visible' => $like && !User::isSelf($model)
        );
        }
