...
abstract class GxActiveRecord extends CActiveRecord
    ...
    public function getRelationLabel($relationName, $n = null, $useRelationLabel = true)
        ...
        foreach ($relNames as $relName)
            ...
            if ($relIndex === $countRelNames)
                ...
                if (isset($labels [$relName])) return $labels [$relName];
                }
    ...
    public function __toString()
        ...
        if (($representingColumn === null) || ($representingColumn === array())) if ($this->getTableSchema()->primaryKey !== null)
                {
    ...
    public function findAllAttributes($attributes = null, $withPk = false, $condition = '', $params = array())
        ...
        if ($attributes === null) $attributes = $this->representingColumn();
        if ($withPk)
            {
    ...
    public static function extractPkValue($model, $forceString = false)
        ...
        if (!is_array($model))
            ...
            if ($forceString && is_array($pk)) $pk = implode(self::$pkSeparator, $pk);
            return $pk;
            } else
            {
            $pks = array();
    ...
    public function fillPkColumnNames($pk)
        ...
        if (count($pk) !== $columnCount) throw new InvalidArgumentException(Yii::t('giix', 'The count of values in the argument "pk" ({countPk}) does not match the count of columns in the composite PK ({countColumns}).'), array(
        '{countPk}' => count($pk),
    ...
    public function saveWithRelated($relatedData, $runValidation = true, $attributes = null, $options = array())
        ...
        try
            ...
            else $transacted = false;

            // Save the main model.
            if (!$this->save($runValidation, $attributes))
                {
                if ($transacted) $transaction->rollback();
                return false;
                }
            ...
            if ($transacted) $transaction->commit();
            }
        catch (Exception $ex)
            {
    ...
    protected function saveRelated($relatedData, $runValidation = true, $batch = true)
        {
        if (empty($relatedData)) return true;

        // This active record can't be new for the method to work correctly.
        if ($this->getIsNewRecord()) throw new CDbException(Yii::t('giix', 'Cannot save the related records to the database because the main record is new.'));
        ...
        foreach ($relatedData as $relationName => $relationData)
            ...
            if (is_array($thisPkValue)) throw new Exception(Yii::t('giix', 'Composite primary keys are not supported.'));
            ...
            if ($newMap !== null)
                ...
                foreach ($currentMap as $currentItem)
                    {
                    if (!in_array($currentItem, $newMap)) $deleteMap [] = $currentItem;
                    }
                // Identify the relations to be inserted.
                foreach ($newMap as $newItem)
                    {
                    if (!in_array($newItem, $currentMap)) $insertMap [] = $newItem;
                    }
                }
            ...
            if (empty($deleteMap) && empty($insertMap)) continue;
            ...
            foreach ($deleteMap as &$deleteMapPkValue) $deleteMapPkValue = array_merge(array(
                    $relatedFkName => $deleteMapPkValue
                        ), array(
                    $thisFkName => $thisPkValue
                ));
            unset($deleteMapPkValue); // Clear reference;
            foreach ($insertMap as &$insertMapPkValue) $insertMapPkValue = array_merge(array(
    ...
    public static function saveMultiple($models, $runValidation = true, $options = array())
        ...
        try
            ...
            foreach ($models as $modelItem)
                ...
                // Merge the options.
                if (isset($modelItem ['modelOptions']) && ($modelItem ['modelOptions'] !== array())) $modelOptions = array_merge($defaultModelOptions, $modelItem ['modelOptions']);
                else $modelOptions = $defaultModelOptions;
                // If set, the global "runValidation" value overrides the model setting.
                if ($runValidation !== null) $modelOptions ['runValidation'] = $runValidation;
                ...
                if ($options ['detectRelations']) $savedModels [get_class($model)] = $model;
                }
    ...
    public static function findRelation($modelClass, $column)
        {
        if (is_string($modelClass)) $staticModelClass = self::model($modelClass);
        else $staticModelClass = self::model(get_class($modelClass));

        if (is_string($column)) $column = $staticModelClass->getTableSchema()->getColumn($column);
    ...
    public function getUrl($action = 'view', $controller = null)
        {
        if ($controller == null) $controllerID = lcfirst(get_class($this));
            ...
            if ($pos) $controllerID = substr($controller, 0, pos);
            }
        $params = array(
            'id' => $this->id
        );
        // add the title parameter to the URL
        if ($this->hasAttribute('title')) $params ['title'] = $this->title;
    ...
    public function linkify($title = null, $controller = null, $action = 'view')
        {
        if ($title == null) $title = (string) $this;
        ...
        foreach ($files as $file)
            ...
            if ($file [0] !== '.' && CFileHelper::getExtension($file) === 'php')
                ...
                if (class_exists($fileClassName) && is_subclass_of($fileClassName, 'GxActiveRecord'))
                    ...
                    if (!$fileClass->isAbstract()) $models [] = $fileClassName;
                    }
                }
            }
        return $models;
    ...
    public function getDataProvider($criteria = null, $pagination = null)
        {
        if ((is_array($criteria)) || ($criteria instanceof CDbCriteria)) $this->cache(1000)->getDbCriteria()->mergeWith($criteria);
...
//                        exit();
    ...
    public function formatDateTime($attribute, $bShowTime = false)
        {
        $format = "F jS, Y ";
        if ($bShowTime) $format = "F jS, Y h:m:s a";
    ...
    public static function rrmdir($dir)
        ...
        if (is_dir($dir))
            ...
            foreach ($objects as $object)
                ...
                if ($object != "." && $object != "..")
                    ...
                    if (filetype($dir . "/" . $object) == "dir")
                        ...
                        {
                        if (unlink($dir . "/" . $object)) echo '<p style="color:red">Removed File : ' . $dir . "/" . $object . '<br /></p>';
                        }
                    }
                }
            reset($objects);
            ...
            if ($dir != Yii::app()->basePath . '/../assets' and $dir != Yii::app()->runtimePath and $dir != Yii::app()->basePath . '/..' . UPLOAD_PATH)
                {
                if (rmdir($dir)) echo '<p style="color:grey">Removed Directory :' . $dir . '<br /></p>';
                }
            }
        }
...
//        
    ...
    public static function getLabelOptions($id = null)
        ...
        if (is_numeric($id)) return $labels [$id % count($labels)];
        return $id;
        }
