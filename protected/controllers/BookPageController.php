...
class BookPageController extends GxController
    {
    ...
    public function actionView($id)
        {
        $model = $this->loadModel($id, 'BookPage');
    ...
    public function actionCreate()
        ...
        $this->performAjaxValidation($model, 'book-page-form');
//        echo '<pre>';
//        print_r($_POST);
//        echo '</pre>';
        ...
        if (isset($_POST['BookPage']))
            {
            $model->setAttributes($_POST['BookPage']);
//            echo '<pre>';
//            print_r($_POST);
//            echo '</pre>';
//            die();
            ...
            $model->saveUploadedFile($model, 'image_right');
            $model->saveUploadedFile($model, 'image_left');
            $model->saveUploadedFile($model, 'img_tr');
            if ($model->save())
                {
...
//        echo '</pre>';
        ...
        $model = $this->loadModel($id, 'BookPage');
        $old_image1 = $model->tab_1_image;
        $old_image2 = $model->tab_2_image;
        $old_image3 = $model->image_right;
        $old_image4 = $model->image_left;
        $old_image5 = $model->img_tr;
...
//            die();
            $model->setAttributes($_POST['BookPage']);
            $new_image1 = $model->saveUploadedFile($model, 'tab_1_image');
            $new_image2 = $model->saveUploadedFile($model, 'tab_2_image');
            $new_image3 = $model->saveUploadedFile($model, 'image_right');
            $new_image4 = $model->saveUploadedFile($model, 'image_left');
            $new_image5 = $model->saveUploadedFile($model, 'img_tr');
            if (!$new_image1)
                {
                $model->tab_1_image = $old_image1;
                }
            if (!$new_image2)
                {
                $model->tab_2_image = $old_image2;
                }
            if (!$new_image3)
                {
                $model->image_right = $old_image3;
                }
            if (!$new_image4)
                {
                $model->image_left = $old_image4;
                }
            if (!$new_image5)
                {
                $model->img_tr = $old_image5;
                }
            if ($model->save())
                {
    ...
    public function actionDelete($id)
        ...
        if (Yii::app()->getRequest()->getIsPostRequest())
            {
            $this->loadModel($id, 'BookPage')->delete();
    ...
    public function actionIndex()
        ...
        $dataProvider = new CActiveDataProvider('BookPage');
    ...
    public function actionSearch()
        ...
        if (isset($_GET['BookPage']))
            {
            $model->setAttributes($_GET['BookPage']);
    ...
    public function actionAdmin()
        {
        $model = new BookPage('search');
        $model->unsetAttributes();
        $this->updateMenuItems($model);

        if (isset($_GET['BookPage'])) $model->setAttributes($_GET['BookPage']);
    ...
    protected function updateMenuItems($model = null)
        ...
        if ($model == null) $model = new BookPage();
