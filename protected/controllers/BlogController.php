...
class BlogController extends GxController {
	...
	public function actionCreate() 
	{
		...
		$this->performAjaxValidation($model, 'blog-form');

		if (isset($_POST['Blog'])) {
			$model->setAttributes($_POST['Blog']);
			$model->saveUploadedFile ( $model, 'thumbnail_file' );
			if ($model->save()) {
	...
	public function actionUpdate($id) 
	{
		...
		if($model)
		{
			$old_image = $model->thumbnail_file;
		}
		//if( !($this->isAllowed ( $model)))	throw new CHttpException(403, Yii::t('app','You are not allowed to access this page.'));
		...
		if (isset($_POST['Blog'])) {
			$model->setAttributes($_POST['Blog']);
			$image = $model->saveUploadedFile( $model, 'thumbnail_file' );
			if(!$image)
			{
				$model->thumbnail_file = $old_image;
			}
			if ($model->save()) {
				$this->redirect(array('view', 'id' => $model->id));
			}
