...
<?php foreach($columns as $column): 
 if (preg_match('/^(status|status_id|state|state_id)/i', $column->name)) :
?>	
...
<?php endif; if (preg_match('/(type_id)/i', $column->name)) :
 ...
 <?php endif; 
	 if (preg_match('/(_time|user_id|manager_id|creater_id)/i', $column->name)) 
	 {
	 	$valid_columns[] = $column;
	 }
	endforeach; ?>
<?php if ( !empty($valid_columns) ) :
	?>	public function beforeValidate()
	{
		if($this->isNewRecord)
		{
	<?php foreach($valid_columns as $column):?>
	<?php if (preg_match('/(create_time|apply_time)/i', $column->name)){
	?>	if ( !isset( $this-><?php echo $column->name  ?> )) $this-><?php echo $column->name  ?> = date( 'Y-m-d H:i:s');
	<?php } if (preg_match('/(manager_id|user_id|creater_id)/i', $column->name)){
	?>	if ( !isset( $this-><?php echo $column->name  ?> )) $this-><?php echo $column->name  ?> = Yii::app()->user->id;			
<?php } endforeach;?>	}else{
	<?php foreach($valid_columns as $column):?>
	<?php if (preg_match('/(modify_time|approve_time)/i', $column->name)){
	?>	if ( !isset( $this-><?php echo $column->name  ?> ))$this-><?php echo $column->name  ?> = date( 'Y-m-d H:i:s');			
<?php } endforeach;?>	}
