...
Yii::import('ext-dev.giix-core.helpers.*');
...
class GiixCrudCode extends CrudCode {
	 ...
	 */
	public $authtype = 'auth_filter_custom';
	/**
	...
	public function generateActiveField($modelClass, $column) {
		if ($column->isForeignKey) {
			...
			'data'=>GxHtml::listDataEx({$relatedModelClass}::model()->findAllAttributes(null, true)),
			))*/" .
			"echo \$form->dropDownList(\$model, '{$column->name}', GxHtml::listDataEx({$relatedModelClass}::model()->findAllAttributes(null, true)))";
		}
		...
		if (strtoupper($column->dbType) == 'TINYINT(1)'
			...
			return "\$form->widget('ext.widgets.clockpick.EClockpick', array(
			'model' => \$model,
			'attribute' => '{$column->name}',
			'value' => \$model->{$column->name},
			'options'          =>array(
		...
		elseif (preg_match('/^to_list|cc_list|user_list$/i', $column->name))
		{
			return "\$this->getAutoCompleteListFromUrl(\$model,'{$column->name}')" ;
		}
		elseif (preg_match('/_list$/i', $column->name))
		{
			return "\$this->getAutoCompleteFromArary(\$model,'{$column->name}',\$model->getStatusOptions())" ;
		}
		elseif (preg_match('/^type|type_id$/i', $column->name))
		{
			return "
			...
			return "\$this->richTextEditor(\$model,'{$column->name}')";
			...
			if ($column->type !== 'string' || $column->size === null)
				...
				return "echo \$form->{$inputField}(\$model, '{$column->name}', array('size'=>80,'maxlength' => {$column->size}))";
			else
	...
	public function generateActiveRelationField($modelClass, $relation) {
		...
		switch ($relationType) {
			case GxActiveRecord::HAS_ONE:
				return "echo \$form->dropDownListGroup(\$model, '{$relationName}', GxHtml::listDataEx({$relationModel}::model()->findAllAttributes(null, true)))";
				break;
			case GxActiveRecord::HAS_MANY:
			case GxActiveRecord::MANY_MANY:
				return "echo \$form->checkBoxListGroup(\$model, '{$relationName}', GxHtml::encodeEx(GxHtml::listDataEx({$relationModel}::model()->findAllAttributes(null, true)), false, true))";
				break;
		}
	}
	...
	public function generateDetailViewAttribute($modelClass, $column) {
		if (!$column->isForeignKey) {
			...
			}elseif (preg_match('/(status|status_id|state|state_id)/i', $column->name))
			{
				...
				'name' => '{$column->name}',
				'type' => 'raw',
				'value'=>\$model->getStatusOptions(\$model->{$column->name}),
				)" ;
			}
			elseif (preg_match('/(type|type_id)/i', $column->name))
			{
				...
				'name' => '{$column->name}',
				'type' => 'raw',
				'value'=>\$model->getTypeOptions(\$model->{$column->name}),
				)" ;
			}
			else
				return "'{$column->name}'";
	...
	public function generateGridViewColumn($modelClass, $column) {
		if (!$column->isForeignKey) {
			...
			elseif (preg_match('/(status|status_id|state|state_id)/i', $column->name))
			{
				return "array(
				'name' => '{$column->name}',
				'value'=>'\$data->getStatusOptions(\$data->{$column->name})',
				'filter'=>$modelClass::getStatusOptions(),
				)" ;
			}
			elseif (preg_match('/(type|type_id)/i', $column->name))
			{
				return "array(
				'name' => '{$column->name}',
				'value'=>'\$data->getTypeOptions(\$data->{$column->name})',
				'filter'=>$modelClass::getTypeOptions(),
				)" ;
			}else
				return "'{$column->name}'";
		} else { // FK.
	...
	public function generateGetPostRelatedData($modelClass, $indent = 1) {
		...
		foreach ($relations as $relationData) {
			...
			if ($relationType == GxActiveRecord::MANY_MANY)
				$result[$relationData[0]] = "php:( isset ( \$_POST['{$modelClass}']['{$relationName}'] ) && \$_POST['{$modelClass}']['{$relationName}'] !== '') ? \$_POST['{$modelClass}']['{$relationName}'] : null ";
		}
...
EOM;
	...
	public function generateActiveRow($modelClass, $column)
	{
		if ($column->isForeignKey) {
			...
			return 
			"\$form->radioButtonListGroup(\$model, '{$column->name}', GxHtml::listDataEx({$relatedModelClass}::model()->findAllAttributes(null, true)))";
		}
		...
		if (strtoupper($column->dbType) == 'TINYINT(1)'
			...
			return "\$form->timepickerGroup(\$model, '{$column->name}',
			 array('hint'=>'Click inside!to select time ',
			 'append'=>'<i class=\"icon-time\" style=\"cursor:pointer\"></i>'))\n";
			
		} elseif (preg_match('/^status|status_id|state|state_id$/i', $column->name))
		{
			return "\$form->dropDownListGroup(\$model,'{$column->name}', array('widgetOptions'=>array('data'=>\$model->getStatusOptions(), 'htmlOptions'=>array('class'=>'input-large'))))";
	
		}
		elseif (preg_match('/^type|type_id$/i', $column->name))
		{
			return "\$form->dropDownListGroup(\$model,'{$column->name}', array('widgetOptions'=>array('data'=>\$model->getTypeOptions(), 'htmlOptions'=>array('class'=>'input-large'))))";

		}
		elseif (preg_match('/^file|_file$/i', $column->name))
		{
			return "\$form->fileFieldGroup(\$model, '{$column->name}')" ;
		}
		elseif (preg_match('/^to_list|cc_list|user_list$/i', $column->name))
		{
			return "\$this->getAutoCompleteListFromUrl(\$model,'{$column->name}')" ;
		...
		elseif (preg_match('/_list$/i', $column->name))
		{
			return "\$form->select2Group(\$model, '{$column->name}',
			array('asDropDownList' => false,
			'options' =>
			array(
			'tags' =>\$model->getTypeOptions(),
			'width' => '40%',
			...
			//return "\$this->getAutoCompleteFromArary(\$model,'{$column->name}',\$model->getStatusOptions())" ;
				...
				return " '';\$code = \$this->richTextEditor() ;\n
					if (\$code == 1) echo \$form->html5EditorGroup(\$model,'{$column->name}', array('class'=>'form-control', 'rows'=>5, 'height'=>'200', 'options'=>array('color'=>true)));\n
					else if (\$code == 2) echo \$form->redactorGroup(\$model,'{$column->name}', array('class'=>'form-control', 'rows'=>5));\n
					else if (\$code == 3) echo \$form->ckEditorGroup(\$model,'{$column->name}', array('options'=>array('fullpage'=>'js:true', 'width'=>'640', 'resize_maxWidth'=>'640','resize_minWidth'=>'320')));\n
					else echo \$form->textAreaGroup(\$model,'{$column->name}',  array('class'=>'form-control', 'rows'=>5));";
